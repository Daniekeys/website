...
export const offeringsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getSingleUpcomingLG.pending, (state) => {
        state.loading = true;
      })
      .addCase(getSingleUpcomingLG.fulfilled, (state,{payload}) => {
          state.loading = false;
            state.singleUpcoming = payload?.data
     
      })
      .addCase(getSingleUpcomingLG.rejected, (state) => {
        state.loading = false;
      })
      .addCase(getAllUpcomingLG.pending, (state) => {
        state.loading = true;
      })
      .addCase(getAllUpcomingLG.fulfilled, (state, {payload}) => {
        state.loading = false;
        state.allUpcomingLiveClasses = payload?.data
       
     
      })
      .addCase(getAllUpcomingLG.rejected, (state) => {
        state.loading = false;
      })
      .addCase(bookLiveGroupOffering.pending, (state) => {
        state.loading = true;
      })
      .addCase(bookLiveGroupOffering.fulfilled, (state, {payload}) => {
        state.loading = false;
...
export const getAllUpcomingLG = createAsyncThunk(
  "getAllUpcomingLg",
  async (payload: any, { rejectWithValue, getState }) => {
   
    try {
      const { data } = await APIService.get(`${url.upLiveComingGroup}?page=${payload?.page}&pageSize=${payload?.pageSize}`
      );
      return data;
    } catch (error: any) {
...
export const getSingleUpcomingLG = createAsyncThunk(
  "getSingleUpcomingLg",
  async (payload: any, { rejectWithValue, getState }) => {
   
    try {
      const { data } = await APIService.get(`${url.getSingleLg}/${payload?.slugURL}`
      );
      return data;
    } catch (error: any) {
      return rejectWithValue(
...
// create a one on one booking session with a coach 
export const bookLiveGroupOffering = createAsyncThunk(
  "bookLiveGroupOffering",
  async (payload: any, { rejectWithValue, getState }) => {
  
    try {
      const { data } = await APIService.post(`${url.bookLiveGroup}/${payload.slugURL}`, payload.data
